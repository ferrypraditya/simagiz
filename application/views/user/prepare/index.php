        ...
        <div class="row mb-1">
          ...
          <div class="col-sm-4 pl-0">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="<?=base_url('prepare?api='.$this->id_t);?>">Select</a></li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
     ...
     <section class="content p-1" id="content">
      ...
      <div class="card" style="height:100% !important">
        ...
        <div class="card-body p-0" style="overflow:scroll;">               
          <table border="1" id="mytable"  class="table table-stripped table-bordered compact">
            <thead>    
...
function pilih(po_no){
  swal({
        ...
        function(){          
         window.location.href ="<?=base_url('prepare/scan?api='.$this->id_t.'&po_no='); ?>"+po_no+""; 
        });            
    }
$('#po_no').autocomplete({
    source: function (request, response) {
      $("#po_no").css({"color": "black","font-size": "12px"});
        $.getJSON("<?=base_url('prepare/searchpart');?>?query=" + request.term+"&api=<?=$this->id_t;?>", function (data) {
          //console.log(data);
...
function logout(){
        swal({
            ...
            function(){
             window.location.href ="<?=base_url('prepare/logout?api='.$this->id_t); ?>";
            });
      }
