        ...
        <div class="row mb-1">
          ...
          <div class="col-sm-4 pl-0">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="<?=base_url('delivery?api='.$this->id_t);?>">Select</a></li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
function pilih(po_no){
  swal({
        ...
        function(){          
         window.location.href ="<?=base_url('delivery/scan?api='.$this->id_t.'&po_no='); ?>"+po_no+""; 
        });            
    }
$('#po_no').autocomplete({
    source: function (request, response) {
      $("#po_no").css({"color": "black","font-size": "12px"});
        $.getJSON("<?=base_url('delivery/searchpart');?>?query=" + request.term+"&api=<?=$this->id_t;?>", function (data) {
          //console.log(data);
...
function logout(){
        swal({
            ...
            function(){
             window.location.href ="<?=base_url('delivery/logout?api='.$this->id_t); ?>";
            });
      }
